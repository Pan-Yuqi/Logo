# -*- coding: utf-8 -*-
"""Utilities for selected-query full attention."""
...
@dataclass
class SelInfo:
    """Metadata for queries packed in sequence and position order.
...
def build_selection(
    ...
    """Build selection metadata for packed variable-length input.

    Args:
        selection: bool mask `[1, total_tokens]`.
        B, T: input batch size and total token count.
    ...
    q_idx = flat.nonzero(as_tuple=False).squeeze(-1)
    cu_k64 = cu_seqlens_k.to(torch.int64)
    seg_id = torch.searchsorted(cu_k64, q_idx, right=True) - 1
    pos_ids = (q_idx - cu_k64[seg_id]).to(torch.int32)
    counts = torch.bincount(seg_id, minlength=Nseq)
